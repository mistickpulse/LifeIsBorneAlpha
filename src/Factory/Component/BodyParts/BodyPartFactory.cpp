...
BodyPart *BodyPartFactory::genBodyPart(const std::string &characterName, BodyPartType type) {
    std::string key = genMapKey(type, characterName);

    if (_BodyPartModel.find(key) != _BodyPartModel.end()) {
        return new BodyPart(*_BodyPartModel[key]);
    }
    std::cerr << "[Error]:Can't create Instance of :" << characterName << " type:" << type._to_string() << std::endl;
    return nullptr;
}

void BodyPartFactory::getModelOfType(std::vector<const BodyPart *> &toFill, BodyPartType type) {
...
void BodyPartFactory::modelRegister(const std::string &characterName, BodyPartType bodyPart, BodyPart data) {
    BodyPart *ptr = new BodyPart(data);

    _BodyPartModel[genMapKey(bodyPart, characterName)] = ptr;
    _BodyPartModelBind[bodyPart].push_back(ptr);
}

const BodyPart *BodyPartFactory::getBodyModel(BodyPartType bodyPart, const std::string &CharacterName) {
